...
public class Save_Panel extends JFrame implements ActionListener {
public JPanel contentPane;
	
	public Connection con = null;
	private String colName[] = { "적금 번호", "적금 금액" , "적금 유형","계좌 번호" };
	...
	//각각의 버튼에서 Source를 가져와 생성자 호출
		   public void actionPerformed(ActionEvent e) {
		      if(e.getSource() == insert) {
		         new Save_InsertDialog();
		      }
		      else if(e.getSource() == selete) {
		    	  Save_DAO dao = new Save_DAO();
		          dao.select(model);
		      }
		      else if(e.getSource() == update) {
		         new Save_UpdateDialog();
		      }
		      else if(e.getSource() == delete) {
		         new Save_DeleteDialog();
		      }
