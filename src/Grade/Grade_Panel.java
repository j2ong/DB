...
public class Grade_Panel extends JFrame implements ActionListener {
	public JPanel contentPane;
	public Connection con = null;
	private String colName[] = { "고객 번호", "계좌 번호","등급" };
		...
		tcmSchedule.getColumn(i).setCellRenderer(tScheduleCellRenderer);
		   ...
		   public void actionPerformed(ActionEvent e) {
		      if(e.getSource() == insert) {
		         new Grade_InsertDialog();
		      }
		      else if(e.getSource() == selete) {
		    	  Grade_DAO dao = new Grade_DAO();
		          dao.select(model);
		      }
		      else if(e.getSource() == update) {
		         new Grade_UpdateDialog();
		      }
		      else if(e.getSource() == delete) {
		         new Grade_DeleteDialog();
		      }
